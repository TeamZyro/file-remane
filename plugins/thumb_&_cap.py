...
async def add_caption(client, message):
    ...
    await PiratesBotRepo.set_caption(message.from_user.id, caption=caption)
    await message.reply_text("**Your Caption Successfully Added ✅**")


@Client.on_message(filters.private & filters.command("del_caption"))
async def delete_caption(client, message):
    caption = await PiratesBotRepo.get_caption(message.from_user.id)
    if not caption:
        return await message.reply_text("**You Don't Have Any Caption ❌**")
    await PiratesBotRepo.set_caption(message.from_user.id, caption=None)
...
async def viewthumb(client, message):
    thumb = await PiratesBotRepo.get_thumbnail(message.from_user.id)
    if thumb:
...
async def removethumb(client, message):
    await PiratesBotRepo.set_thumbnail(message.from_user.id, file_id=None)
...
async def addthumbs(client, message):
    ...
    await PiratesBotRepo.set_thumbnail(
